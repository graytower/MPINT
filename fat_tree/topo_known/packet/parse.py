...
class parse():
    def filter(self, pkt_raw):  # filter int packet
        red_int = redis.Redis(unix_socket_path='/var/run/redis/redis.sock',db=2)
        ...
        if ethernet == 8224:
            ...
            pkt = struct.unpack("!B%ds" % (pkt_len - 14 - 1), pkt[1])
            int_option = pkt[0]
    ...
    def parse_int(self, red_int, pkt):
        ...
        key = str(sw_id)
        red_int.hset('topo', key, '')
        red_int.set(key, str_int)
