...
parser MyParser(packet_in packet,
	...
	state parse_ethernet {
        ...
        transition select(hdr.ethernet.etherType) {
            ...
            //ETHER_TYPE_MPINT: parse_branch_id;
            //default: accept;
        }
	}
	...
	state parse_ipv4 {
		...
		transition parse_branch_id;
	}
    
	state parse_branch_id {
	    packet.extract(hdr.branch_sw_id);
...
control MyEgress(inout headers hdr,
    ...
    action del_int_info(bit<8> sw_id) {
        hdr.int_option.int_total_num = 0;
        hdr.int_info.pop_front(MAX_HOPS);
        hdr.branch_sw_id.last_branch_sw_id = (bit<8>)sw_id;
    }
