...
struct headers {
    ethernet_t          ethernet;
	ipv4_t              ipv4;
    pathoption_t        path_option;
    pathhdr_t[MAX_HOPS] path_info;
...
parser MyParser(packet_in packet,
	...
	state parse_ethernet {
        ...
        transition select(hdr.ethernet.etherType) {
            ...
            //ETHER_TYPE_MPINT: parse_path_option;
            default: accept;
        }
	}
	...
	state parse_ipv4 {
		...
		transition parse_path_option;
	}
    
    state parse_path_option {
	    packet.extract(hdr.path_option);
		meta.path_info_remaining = hdr.path_option.path_total_num;
        transition select(meta.path_info_remaining) {
            0 : parse_int_option;
            default: parse_path_info;
        }
	}
    
    state parse_path_info {
	    packet.extract(hdr.path_info.next);
		meta.path_info_remaining = meta.path_info_remaining - 1;
		transition select(meta.path_info_remaining) {
            0 : parse_int_option;
            default: parse_path_info;
        }
...
control MyEgress(inout headers hdr,
    ...
    action add_path_info(bit<8> sw_id) {
        hdr.path_option.path_total_num = hdr.path_option.path_total_num + 1;
        hdr.path_info.push_front(1); //path路径倒序
        hdr.path_info[0].setValid();
        hdr.path_info[0].sw_id = (bit<8>)sw_id;
    }
